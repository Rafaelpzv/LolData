  ...
  return (
    ...
    >
      <Badge
        ...
        className="num h-10 min-w-12 justify-center self-start px-2 text-lg font-semibold sm:row-span-2 sm:h-12 sm:min-w-14 sm:text-xl"
      >
      ...
      <div className="col-span-2 space-y-3 sm:col-span-1 sm:col-start-2">
        {traits.length > 0 && (
          <ul aria-label={t("traits")} className="flex flex-wrap gap-1.5">
            {traits.map((trait) => (
        ...
        {units.length > 0 && (
          <ul aria-label={t("units")} className="flex flex-wrap gap-x-2 gap-y-4 pb-2 pt-1">
            {units.map((unit, i) => (
...
function TraitChip({ trait }: { trait: TftTrait }) {
  ...
  return (
    <li
      title={t("traitTitle", { name, style: styleLabel, current: trait.tier_current, total: trait.tier_total })}
      className={cn(
...
function UnitIcon({ unit, ddragonVersion }: { unit: TftUnit; ddragonVersion: string | null }) {
  ...
  return (
    <li className="relative" title={details}>
      <IconFrame
