/**
 * Champion id -> display name (canonical Riot English names).
 * Static list so filters work without fetching Data Dragon.
 */
...
export function championName(id: number | string | null | undefined): string | null {
  if (id === null || id === undefined) return null;
  return CHAMPIONS.find((c) => c.id === String(id))?.name ?? null;
}

/** URL param -> champion id. Accepts numeric ids ("103") and legacy name slugs ("kaisa"). */
export function championIdFromParam(param: string | null | undefined): string | null {
  if (!param || param.toLowerCase() === "all") return null;
  if (CHAMPIONS.some((c) => c.id === param)) return param;
  const slug = param.toLowerCase();
  return (
    CHAMPIONS.find((c) => c.name.replace(/\s+/g, "").replace(/['.]/g, "").toLowerCase() === slug)?.id ??
    null
  );
}
